...
export function HistoryChart({ points }: { points: HistoryPoint[] }) {
  const hybrid = points.filter((point) => point.mode === "HYBRID");
  const snapshots = [...new Set(hybrid.map((point) => point.snapshot_id))];
  const teams = [...new Set(hybrid.map((point) => point.team_id))];
  if (snapshots.length < 2) return <p className="empty">Se necesitan dos snapshots para dibujar evolución.</p>;
  const width = 900;
  const height = 250;
  const maximum = Math.max(1, ...hybrid.map((point) => point.champion_probability));
  return (
      ...
      <svg viewBox={`0 0 ${width} ${height}`} role="img" aria-label="Evolución histórica de probabilidad de campeón">
        ...
        {teams.map((team) => {
          const teamPoints = snapshots.map((snapshot, index) => {
            const point = hybrid.find((item) => item.snapshot_id === snapshot && item.team_id === team);
