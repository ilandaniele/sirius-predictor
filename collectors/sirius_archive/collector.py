...
class SiriusBloggerArchiveCollector(Collector):
    """Capture all Blogger posts as a reviewable, non-model-input corpus."""
    ...
    def _page_url(self, start_index: int) -> str:
        base = "https://astrologiadeportivaa.blogspot.com/feeds/posts/default"
        query = urlencode(
    ...
    def fetch(self) -> bytes:
        ...
        if len(payload) > self.client.max_bytes:
            raise ValueError("normalized Sirius archive exceeds configured snapshot limit")
        return payload
...
def sirius_archive_collector_from_config(
    record: dict[str, object],
) -> SiriusBloggerArchiveCollector:
    url = str(record["url"])
    host = urlsplit(url).hostname
    if host is None:
        raise ValueError("Sirius archive source URL has no hostname")
    ...
    return SiriusBloggerArchiveCollector(spec)
